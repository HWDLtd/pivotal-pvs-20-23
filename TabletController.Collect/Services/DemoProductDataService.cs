...
namespace TabletController.Collect.Services
{
    public class DemoProductDataService : IProductDataService
    {
        private List<Category> _categories;
        private List<Product> _products;
        private List<string> _adminCodes;
        ...
        public void LoadData()
        {
            lock (_lockObject)
            {
                try
                {
                    ...
                    _adminCodes = demoData.AdminCodes ?? new List<string>();
                    System.Diagnostics.Debug.WriteLine($"Loaded {_categories.Count} categories, {_products.Count} products, and {_adminCodes.Count} admin codes");
                }
                catch (Exception ex)
                {
                    ...
                    if (_adminCodes == null) _adminCodes = new List<string>();
                }
            }
        }
        ...
        public List<string> GetAdminCodes()
        {
            lock (_lockObject)
            {
                return new List<string>(_adminCodes);
            }
        }
        ...
        public bool IsAdminCode(string code)
        {
            ...
            lock (_lockObject)
            {
                return _adminCodes.Any(ac => ac.Equals(code, StringComparison.OrdinalIgnoreCase));
            }
        }
