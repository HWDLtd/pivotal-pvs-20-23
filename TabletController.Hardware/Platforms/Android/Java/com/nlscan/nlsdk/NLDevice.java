    ...
    public NLDevice(DevClass classType) {
        ...
        switch (classType) {
            ...
            case DEV_COMPOSITE:
                ...
                break;
            case DEV_UART:
                // UART not supported - use USB connection
                Log.e(TAG, "UART mode not supported!");
                break;
            default:
...
//	}
    ...
    private int updateDevice(byte[] data, NLCommStream.DevClass firmwareType, UpdateInfo[] updateInfos, int total, NLUpdateListner listner) {
        ...
        // Note: UART mode not supported, always use USB
        if (firmwareType == NLCommStream.DevClass.DEV_MCU) {
            //curCommStream.close(mContext);
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
        ...
        for (int idx = 0; idx < total; ++idx) {
            ...
            // 2. enter update mode
            // Note: UART mode not supported - skip baud rate change for USB
            ...
            // 3.  switch baud rate (optional) - Note: UART mode not supported, skip for USB
